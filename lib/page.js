...
function getPageTemplate(pagePath) {
    return assetsCfg.getPageAsset(pagePath, 'bemhtml');
}

function getPageBEMJSON(pagePath, globalParams, data, onlyBody) {
    return assetsCfg.getPageAsset(pagePath, 'priv').then(function(priv) {
...
function buildPageGlobalParams(pagePath, req) {
    return {
        assetsPath : [
            'host' in assetsCfg? assetsCfg.host : (req.protocol + '://' + req.host),
...
module.exports = function(pagePath, req, resp, next) {
    logger.verbose('page "%s" is running', pagePath);
    jaggi.create(
        ...
        },
        {
            ...
            .run()
                ...
                .then(
                    function(res) {
                        resp.send(res);
                        logger.verbose('page "%s" completed', pagePath);
                    },
                    function(e) {
                        next(e);
                    });
